...
JNIEXPORT jlong JNICALL Java_com_alibaba_mnnllm_android_ChatSession_initNative(JNIEnv* env, jobject thiz, jstring modelDir,
                                                                                    ...
                                                                                    jboolean is_diffusion, jstring prompt) {
    const char* model_dir = env->GetStringUTFChars(modelDir, 0);
    const char* cprompt = nullptr;
    if(prompt != nullptr) {
        cprompt = env->GetStringUTFChars(prompt, 0);
    }
    ...
    history.clear();
    if(prompt != nullptr) {
        history.emplace_back("system", cprompt);
    } else {
...
//    llm->response(history, &output_ostream, "<eop>", 1);
    llm->response(history, &output_ostream, nullptr, 0);
//    while (!stop_requested && llm->getState().gen_seq_len_ < 512) {
    while (!stop_requested && !llm->stoped()) {
        llm->generate(1);
    }
